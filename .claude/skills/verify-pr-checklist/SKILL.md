...
3. ログイン後、意図したページ（ダッシュボード等）にリダイレクトされ、`/login` に戻されないことを確認してから、手順3以降の検証に進む。
...
- middlewareにより、ログインしていないと全ページが `/login` にリダイレクトされる → 検証を始める前に手順2.5の通り必ずログイン済みセッションを確立すること
