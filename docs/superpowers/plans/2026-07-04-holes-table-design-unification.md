...
- [x] **Step 1: globals.css の末尾に以下を追記する**
...
- [x] **Step 2: レイアウトセクションの見出し・サマリーをTailwindクラスに置き換える**
...
- [x] **Step 3: テーブルの閉じタグに対応する `</div>` を追加する**
...
- [x] **Step 4: グループヘッダー行（`colSpan`）をTailwindクラスに置き換える**
...
- [x] **Step 5: 項目行（`<th>Hole</th>` 等）をTailwindクラスに置き換える**
...
- [x] **Step 6: データ行のセルをTailwindクラスに置き換える**
