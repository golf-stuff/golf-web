...
- [ ] **Step 1: globals.css を開いて現在の内容を確認する**
...
- [ ] **Step 3: 開発サーバーを起動してスタイルが読み込まれることを確認する**
...
- [ ] **Step 4: layouts/page.tsx を書き換える**
...
- [ ] **Step 5: holes/page.tsx（ホール定義）を書き換える**
...
- [ ] **Step 6: holes/ui.tsx（クライアントコンポーネント）のスタイルを更新する**
...
- [ ] **Step 3: ホール入力ページのヘッダー・ボタン部分を Tailwind 化する**
...
- [ ] 下線スタイルの input / select が表示される
- [ ] `btn-primary`（黒背景）と `btn-secondary`（白背景・枠）が視覚的に区別できる
- [ ] ダッシュボード（`/`）のカードと各ページのカードが同じ見た目（`rounded-xl border-gray-200`）になっている
- [ ] モバイルサイズ（幅375px）でも崩れない
