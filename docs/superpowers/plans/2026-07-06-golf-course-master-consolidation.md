...
- [ ] **Step 3: Prismaスキーマに `user_role` enumと `MstUser.role` を追加**
...
- [ ] **Step 3: Prisma Clientの型が更新されたことを確認**
...
- [ ] **Step 1: `app/golf-courses/actions.ts` を全面的に更新**
...
- [ ] **Step 2: 新規作成画面に都道府県・市区町村フィールドを追加**
...
- [ ] **Step 3: 編集画面から所有者チェックを削除し、都道府県・市区町村フィールドを追加**
...
- [ ] **Step 1: `/rounds/new` のゴルフ場一覧取得からuserIdフィルタを削除**
...
- [ ] **Step 2: `/rounds/import` のゴルフ場一覧取得からuserIdフィルタを削除し、空データ時の案内文言を更新**
...
- [ ] **Step 1: 既存テストスイート全体を実行**

Run: `npx vitest run`
Expected: 全テストPASS

- [ ] **Step 2: ローカルSupabaseを起動し、動作確認用の管理者ユーザーを用意**
...
- [ ] **Step 4: 一般ユーザー（role: user）として動作確認**
