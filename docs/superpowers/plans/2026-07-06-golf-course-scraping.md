...
- 一覧ページ（都道府県ごとのコース一覧）の実サンプルはまだ取得できていない。以下のURLパターンは判明している:
  - GDO: `https://reserve.golfdigest.co.jp/course-guide/area/{都道府県コード}/`（`{都道府県コード}`はJIS都道府県コード2桁、例: `01`=北海道）
  - 楽天GORA: `https://gora.golf.rakuten.co.jp/doc/area/{都道府県スラッグ}/`（`https://gora.golf.rakuten.co.jp/doc/area/`に都道府県ごとのリンク一覧があり、例: `hokkaido`）
  - じゃらんゴルフ: `https://golf-jalan.net/course/{地方 or 都道府県スラッグ}/...`（`https://golf-jalan.net/`に全国一覧があるが、都道府県によってネスト構造が異なる。例: `hokkaido` → `https://golf-jalan.net/course/hokkaido/`（都道府県が最上位）、栃木 → `https://golf-jalan.net/course/kanto/tochigi/`（地方配下にネスト）。この不整合を吸収する必要がある
  - 上記3サイトとも、一覧ページ自体のHTML構造（コース詳細ページへのリンクの見つけ方、ページネーションの有無）は未確認。Task 9実装前に、各サイトの東京都の一覧ページ（またはコース件数の少ない都道府県）のサンプルを人手で取得し、フィクスチャ化するステップを設ける
...
- Create: `src/lib/scraping/parsers/__tests__/fixtures/{gdo,rakuten-gora,jalan-golf}/area-tokyo.txt`（一覧ページの実サンプル。**着手前にユーザーから提供してもらう**）
...
> **着手前提:** 本Task着手前に、以下3サイトの東京都（または他の都道府県）の一覧ページの実HTML/テキストサンプルをユーザーから提供してもらい、フィクスチャ化する。詳細ページ同様、実構造を見ずに селектор等を推測しない

- [ ] **Step 1: 都道府県名 → サイトごとのURLパスセグメントの変換テーブルを実装する**

`src/lib/scraping/prefectureSlug.ts`に、47都道府県について以下を持つテーブルを定義する:
- GDO用: JIS都道府県コード2桁（例: 東京都→`13`）
- 楽天GORA用: ローマ字スラッグ（例: 東京都→`tokyo`）
- じゃらんゴルフ用: 地方スラッグ + 都道府県スラッグのペア（例: 東京都→`{ region: "kanto", prefecture: "tokyo" }`。ただし北海道など地方に属さない都道府県は`region: null`とし、その場合`https://golf-jalan.net/course/{prefecture}/`の形になる）

- [ ] **Step 2: 各サイトの`listCourseUrls`を、Step 1のURLとフィクスチャに基づいて実装する**

`fetchAndExtractText`（または生HTML取得＋`extractLinks`）で一覧ページを取得し、コース詳細ページのURLパターンにマッチするリンクを抽出する。ページネーションがある場合は次ページのURL/カーソルを`ScrapingJobProgress.cursor`に保存できる形にする（実際のページネーション構造はフィクスチャ取得時に確認する）

- [ ] **Step 3: フィクスチャに対するテストを書く**

一覧ページのフィクスチャから、期待されるコース詳細URLの集合が抽出できることを確認する
...
- 一覧ページの探索（Task 9）はユーザーから実URLパターンの提供を受けたが、実HTML構造（リンクの見つけ方・ページネーション）は未確認のため、着手前に実サンプルをフィクスチャとして取得するステップを明示した
