...
- [x] **Step 3: `ParsedScore` の型を拡張し `parseGdoFullRound` を実装する**
...
- [x] **Step 4: `parseGdoScoreText` の `ParsedScore` 変更に伴う既存コードの修正**
...
> **注記（2026-07-05追記）**: このタスクは docs/superpowers/plans/2026-07-04-gdo-import-merged.md に統合され、そちらの設計・実装（コミット `64c5d34`, `5396367`, `cabfe6f`）で完了しています。以下のステップは本プランの記述通りには実行されていないため、チェックは付けません。
...
> **注記（2026-07-05追記）**: Task 1（パーサー拡張）は完了済みですが、Task 2・3 は docs/superpowers/plans/2026-07-04-gdo-import-merged.md に統合されて完了しています。以下のチェックリストは本プランの実装内容とは異なる設計（upsert方式・getCurrentUser()認証）で達成されているため、チェックは付けません。
