...
- [ ] **Step 2: docker-compose.ymlを削除する**

```bash
git rm docker-compose.yml
```

- [ ] **Step 3: supabase initを実行する**
...
- [ ] **Step 6: PrismaマイグレーションをローカルSupabaseに適用する**
...
- [ ] **Step 1: schema.prismaにdirectUrlを追加する**
...
- [ ] **Step 2: Supabaseクライアントパッケージをインストールする**
...
- [ ] **Step 1: Server Component/Action用Supabaseクライアントを作成する**
...
- [ ] **Step 2: ブラウザ用Supabaseクライアントを作成する**
...
- [ ] **Step 1: ログイン・ログアウトServer Actionを作成する**
...
- [ ] **Step 2: ログインフォーム（クライアントコンポーネント）を作成する**
...
- [ ] **Step 3: ログインページ（Server Component）を作成する**
...
- [ ] **Step 4: HeaderNavにログアウトボタンを追加する**
...
- [ ] **Step 1: golf-courses/actions.tsを修正する**
...
- [ ] **Step 3: rounds/import/actions.tsを修正する**
...
- [ ] **Step 1: dashboard/queries.tsにuserId引数を追加する**
...
- [ ] **Step 2: app/page.tsx（ダッシュボード）を修正する**
...
- [ ] **Step 3: app/rounds/page.tsxを修正する**
...
- [ ] **Step 4: app/rounds/new/page.tsxを修正する**
...
- [ ] **Step 5: app/rounds/import/page.tsxを修正する**
...
- [ ] **Step 6: app/golf-courses/page.tsxを修正する**
...
2. Project Settings → Database → Connection string から以下2つを控える
   ...
   - **Direct connection**（port 5432）→ `DIRECT_URL`として使用
...
- [ ] **Step 2: 本番プロジェクトにマイグレーションを1回だけ手動適用する（初回のみ）**

Task 10でGitHub Actions自動化を構築するが、初回動作確認のため一度だけローカルから手動実行する:

```bash
cd golf-web
DIRECT_URL="<Step1で控えたDirect connection文字列>" node node_modules/prisma/build/index.js migrate deploy
```

Expected: 全マイグレーションが本番プロジェクトに適用される（`.env.local`は変更しない）

- [ ] **Step 3: Google/X OAuthプロバイダーを本番プロジェクトに設定する**
...
- [ ] **Step 4: Vercelプロジェクトを作成する**

1. https://vercel.com でGitHubの`golf-stuff`リポジトリをImportする
2. **Root Directory**を`golf-web`に設定する
3. Project Settings → Git → Production Branch を`main`に設定する
4. **Project Settings → Git → 「Deploy Hooks」機能を使うため、この時点ではまだ何もpushしない**（Task 10で無効化するため）

- [ ] **Step 5: Vercel環境変数を設定する**
...
- [ ] **Step 6: Vercel Deploy Hookを発行する**
...
- [ ] 本番DBにマイグレーションが適用済み
- [ ] Google/X OAuthプロバイダーが有効化されている
- [ ] Vercelプロジェクトが作成され、Root Directoryが`golf-web`に設定されている
- [ ] Vercel環境変数（`DATABASE_URL`等）が登録されている
- [ ] Vercel Deploy HookのURLを控えている
...
- [ ] **Step 1: GitHub Secretsを登録する**

リポジトリの Settings → Secrets and variables → Actions で以下を登録する:

```
PROD_DIRECT_URL       = <Task 9 Step1で控えたDirect connection文字列>
...
- [ ] 本番URL（Vercel）にアクセスすると`/login`にリダイレクトされる
- [ ] Supabaseダッシュボードで作成したアカウントでメール+パスワードログインができる
- [ ] Google/Xでのログインができる
- [ ] ログイン後、ダッシュボード・ラウンド一覧・ゴルフ場一覧が表示される（初回は空データ）
- [ ] ゴルフ場・レイアウト・ホール登録ができる
- [ ] GDOスコアインポートができる
- [ ] ログアウトすると`/login`に戻る
- [ ] スキーマ変更を含むPRをmainにマージすると、GitHub Actions経由で自動的に本番DBへマイグレーションが適用され、Vercelへデプロイされる
